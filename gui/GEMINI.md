...
- **`main_window.py` (`MainWindow`)**: 
    - **作用**: GUI 应用的入口和主容器。
    - **核心职责**: 初始化并管理 Worker 线程；包含所有 `handle_*` 槽函数，作为后台请求的UI响应中心；使用 `QSplitter` 和 `QTabWidget` 构建主界面的左右布局。**它集成了丰富的用户反馈机制：在状态栏中通过进度条和耗时标签显示任务进度；在执行长耗时任务时自动切换为等待光标；并在任务失败时弹出明确的错误对话框。**
...
- **`image_loader.py` (`ImageLoader`)**: **[新]**
    ...
    - **核心功能**: 在 `SelectionDialog` 中被用来加载和显示游戏封面图。它支持**两级缓存**：一个用于快速访问的内存缓存，以及一个位于 `cache/images` 目录下的**持久化磁盘缓存**。这确保了即使在程序重启后，已下载过的图片也能被迅速加载，极大地提升了性能和用户体验。
