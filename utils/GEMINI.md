...
- **`similarity_check.py`**: 
    - **作用**: **[功能增强]** 提供了一套完整的游戏查重解决方案。
    - **核心类 `SimilarityChecker`**:
        - **混合匹配算法**: 通过 `n-gram` 快速筛选候选项，然后使用 `rapidfuzz` 库进行精确的相似度计算。
        - **智能加分**: 内置了**子字符串加分**逻辑，能够将带有 `【限定版】`、`（下载版）` 等前后缀的标题与它们的纯净版正确识别为高度相似，极大地提高了查重准确率。
    - **缓存机制**: 包含 `load_cache_quick`, `save_cache` 等函数，通过将 Notion 中的所有游戏标题缓存到本地 (`cache/game_titles_cache.json`)，显著减少了不必要的 API 请求，加快了查重速度。