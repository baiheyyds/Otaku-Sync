...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        mapping_editor_widget = self.create_mapping_editor()
        controls_splitter.addWidget(mapping_editor_widget)
    ...
    def start_search_process(self):
        if self.worker and self.worker.isRunning():
            QMessageBox.warning(self, "任务正在进行", "请等待当前游戏录入任务完成。\n")
            return
        ...
        self.worker = GameSyncWorker(keyword=keyword, manual_mode=manual_mode, shared_context=self.shared_context)
        self.worker.context_created.connect(self.set_shared_context)
        self.worker.selection_required.connect(self.handle_selection_required)
    ...
    def handle_property_type(self, request_data):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self.worker.set_interaction_response(selected_type)
        else:
            self.worker.set_interaction_response(None)

    def handle_selection_required(self, choices, title, source):
        if not choices:
            project_logger.warn("未找到任何结果。\n")
            self.worker.set_choice(None)
            return
        ...
        if result == QDialog.Accepted:
            ...
            self.worker.set_choice(choice_index)
        elif result == 2:
            project_logger.info("用户选择切换到 Fanza 搜索...\n")
            self.worker.set_choice("search_fanza")
        else:
            project_logger.info("用户取消了选择。\n")
            self.worker.set_choice(-1)
    ...
    def closeEvent(self, event):
        ...
        if self.shared_context:
            ...
            try:
                ...
                project_logger.error(f"清理资源时发生错误: {e}")
